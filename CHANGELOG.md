...
- Added `commitment-service.test.ts` (30 cases): `extractCommitment` positive verb patterns, negative patterns ("I'll await", "I'll be brief", "I'll second"), noun-ish guard, fenced/quoted-block rejection, `extractCompletion` path extraction + URL/absolute-path rejection.
- Total suite: **767 passing** (was 704 before the post-review fixes).

### Changed
- **Schema migrations** — `022_todos_commitment_fields` adds commitment columns + two indexes; `023_todos_idle_progress_index` adds the partial covering index for the idle sweeper hot path.
...
- **`SocketEventNames`** adds `agentAck`, `mirrorSuppressed`, `echoSuppressed`, `commitmentCreated`, `commitmentUpdated`, `commitmentExpired`.
