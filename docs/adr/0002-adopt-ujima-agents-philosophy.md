...
8. **Intelligence-first.** Anything that can be abstracted to an agent decision should be — channel slug choice, "Run as task" suggestion, referee selection, mention fan-out, summarization, retry vs. escalate. Hardcode only the invariants: auth, approval gates, path-scope rejection, schema validation, budget caps, rate limits, idempotency keys. Every LLM-decided behavior has a deterministic fallback if the call fails.
