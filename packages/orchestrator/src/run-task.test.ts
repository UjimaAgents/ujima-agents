...
describe('orchestrator runTask — manual mode + concurrent execution', () => {
  ...
  it('slim mode checkpoints completed stages and resumes from the last completed stage', async () => {
    ...
    const firstModels = new Map<string, LanguageModel>([
      [
        'sr-designer',
        lm({
    ...
    const firstHandle: ReturnType<typeof runTask> = runTask(
      {
        ...
        getMCPConnection: () => mcp,
        getModel: (agent) => getOrThrow(firstModels, agent.id),
        eventBus: bus,
        context: db.context,
        audit: db.audit,
        permissions,
    ...
    const resumeModels = new Map<string, LanguageModel>([
      [
        'sr-designer',
        createLanguageModelFromLegacyProvider(
          {
    ...
    const secondHandle = runTask(
      {
        resolveAgent: (id) =>
          id === 'sr-designer' ? srDesigner : id === 'jr-designer' ? jrDesigner : undefined,
        getMCPConnection: () => mcp,
        getModel: (agent) => getOrThrow(resumeModels, agent.id),
        eventBus: bus,
        context: db.context,
        audit: db.audit,
        permissions,
