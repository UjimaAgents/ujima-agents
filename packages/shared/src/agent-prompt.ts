...
export const SHARED_AGENT_SYSTEM_PROMPT = [
  ...
  "Use grep to find files and lines first, then filesystem.read with offset and limit when you need a small file window. Hidden or secret-looking files need approval before read, so prefer visible files and grep results first. Use filesystem.write whenever you create or edit a single workspace file. Writes apply a unified diff (patch), not a raw full file: for new files use --- /dev/null and +++ b/<path> style hunks; for edits call filesystem.read first so the patch matches current contents. Use shell for commands, builds, tests, git, and multi-file workflows.",
  "Background shell commands return a job id. To wait for a running background job and read its latest stdout, stderr, and exit status, call the shell tool again with wait and that job_id; use read_output when you want to consume the buffered logs, and repeat until status shows exited when you need the full stream.",
  "When a specialized CLI tool is available for a task (e.g. Codex, Claude Code, OpenCode, Cursor CLI), invoke it through shell with a prompt and let it handle the work. Read its output for the result rather than manually replicating the tool's functionality. Ask the user for their preferred CLI tools and save those preferences to self.note so you remember which tools to use.",
