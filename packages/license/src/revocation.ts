...
export async function refreshRevocations(
  ...
  // Fetch + persist. ONLY advance fetchedAt on a fully successful
  // fetch — pre-fix we marked a failed/missing fetch as fresh which
  // (a) suppressed retries for 24h after one transient blip, and
  // (b) locked in an empty ids list as "fresh" on first-ever offline
  // run. On failure we leave the prior cache untouched so the next
  // call retries; for the force path we surface the error so
  // `ujima license refresh` can fail visibly instead of pretending
  // it worked.
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(body.ids)) {
        ...
        fetchErr = new Error(`unexpected payload from ${REVOCATION_URL}`);
      }
    } else {
      fetchErr = new Error(`revocation feed returned HTTP ${res.status}`);
    }
  } catch (err) {
  ...
  if (ids === null) {
    if (options.force) {
      throw fetchErr ?? new Error('revocation refresh failed');
    }
    // Silent best-effort: leave prior cache + memo intact.
    return;
  }
