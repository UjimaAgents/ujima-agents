...
import { channelRoom, memberRoom, orgRoom, threadRoom, type RealtimeService } from "../realtime.ts";
...
export class ConversationService {
  ...
  publishMessage(message: Message) {
    ...
    const channel = message.channelId ? this.repo.getChannel(message.organizationId, message.channelId) : null;
    const rooms =
      channel?.kind === "dm"
    ...
    this.realtime.emit(
      channel?.kind === "dm" ? SocketEventNames.dmMessage : message.channelId ? SocketEventNames.channelMessage : SocketEventNames.threadMessage,
      channel?.kind === "dm"
