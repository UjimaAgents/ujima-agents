...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(parseApiError(body, "Unable to get heartbeat."), {
...
export async function PATCH(
  ...
  try {
    ...
    const body = await response.json().catch(() => null);
    if (!response.ok) {
      return NextResponse.json(parseApiError(body, "Unable to update heartbeat."), {
        status: response.status,
      });
    }
    return NextResponse.json(body, { status: response.status });
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      const body = await response.json().catch(() => null);
      return NextResponse.json(parseApiError(body, "Unable to delete heartbeat."), {
        status: response.status,
      });
    }
