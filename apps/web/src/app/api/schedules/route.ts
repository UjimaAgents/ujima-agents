...
export async function GET() {
  try {
    const response = await daemonFetch("/api/schedules", {}, await getSessionTokenFromCookie());
    const body = await response.json().catch(() => null);

    if (!response.ok) {
      return NextResponse.json(
        parseApiError(body, "Unable to list scheduled jobs."),
...
export async function POST(request: Request) {
  try {
    ...
    const response = await daemonFetch("/api/schedules", {
      method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json(
        parseApiError(body, "Unable to create scheduled job."),
        { status: response.status },
      );
    }
