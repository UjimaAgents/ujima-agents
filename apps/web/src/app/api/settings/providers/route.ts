...
export async function GET(request: Request) {
  try {
    ...
    const response = await daemonFetch(
      `/api/settings/providers?organizationId=${encodeURIComponent(organizationId)}`,
      {},
    ...
    if (!response.ok) {
      return NextResponse.json(
        parseApiError(body, "Unable to fetch providers."),
...
export async function POST(request: Request) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        parseApiError(body, "Unable to update providers."),
        { status: response.status },
      );
    }
